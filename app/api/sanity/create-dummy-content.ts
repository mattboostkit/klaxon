...
export async function GET() {
  try {
    ...
    // Create blog posts with placeholder image references
    // Note: In a real scenario, you would upload images to Sanity first and use their references
    const dummyImageRef = 'image-placeholder-reference'; // This is a placeholder
    ...
    // Try to create posts but handle errors if image references don't exist
    let createdPosts = [];
    try {
      createdPosts = await Promise.all(
        posts.map(post => client.create(post))
      );
    } catch (error) {
      console.error('Error creating posts with image references:', error);
      
      // Try creating posts without image references
      ...
      createdPosts = await Promise.all(
        postsWithoutImages.map(post => client.create(post))
      );
    }
